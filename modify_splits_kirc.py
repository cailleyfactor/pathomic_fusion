...
for k in range(1, 16):
    ...
    # Handle non-numeric data conversion here if needed
        ...
        data[split]['x_omic'] = np.take(data[split]['x_omic'],index_list, axis=0)
        data[split]['x_path'] = np.take(data[split]['x_path'],index_list, axis=0)
        data[split]['e'] = np.take(data[split]['e'],index_list, axis =0)
        data[split]['t'] = np.take(data[split]['t'],index_list, axis =0)
        data[split]['g'] = np.take(data[split]['g'],index_list, axis =0)

# Create the directory if it doesn't exist
os.makedirs('./data/TCGA_KIRC/splits/', exist_ok=True)

# Create the file and dump the data
with open('./data/TCGA_KIRC/splits/KIRC_st_0_clin.pkl', 'wb') as file:
