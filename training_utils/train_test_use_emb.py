...
from data_loaders import UseEmbeddingsDatasetLoader
from networks_use_emb import define_net, define_reg, define_optimizer, define_scheduler
...
def train(opt, data, device, k):
    ...
    custom_data_loader = (
        UseEmbeddingsDatasetLoader(opt, data, split="train", mode=opt.mode)
        if opt.use_vgg_features
        else UseEmbeddingsDatasetLoader(opt, data, split="train", mode=opt.mode)
    )
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter + opt.niter_decay + 1)):
        ...
        for batch_idx, (x_path, x_grph, x_omic, censor, survtime, grade) in enumerate(
            ...
            # draw = 1
            directory = './GBMLGG_embeddings'
            emb_x_paths = []
            if "path" in opt.mode:
                for png_tensor in x_path:
                    png = png_tensor.item()
                    png_filename = png.split("/")[-1] + ".npy"
                    file_path = os.path.join(directory, str(draw))
                    png_path = os.path.join(file_path, png_filename)
                    pat_emb = np.load(png_path)
                    emb_x_paths.append(pat_emb)

                path_embeddings = np.vstack(emb_x_paths)
                x_path = torch.tensor(path_embeddings, dtype=torch.float32)
        ...
        if opt.measure or epoch == (opt.niter + opt.niter_decay - 1):
            ...
            file_path = os.path.join(dir_path, "%s_%d%s%d_pred_test.pkl" % (opt.model_name, k, use_patch, epoch))
...
def test(opt, model, data, split, device):
    ...
    custom_data_loader = (
        UseEmbeddingsDatasetLoader(opt, data, split, mode=opt.mode)
        if opt.use_vgg_features
        else UseEmbeddingsDatasetLoader(opt, data, split=split, mode=opt.mode)
    )
