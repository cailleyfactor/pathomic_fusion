...
def define_net(opt, k):
    ...
    if opt.mode == "path":
        ...
        raise NotImplementedError('model [%s] is not implemented' % opt.mode)
...
class GraphNet(torch.nn.Module):
    def __init__(self, features=1036, nhid=128, grph_dim=32, nonlinearity=torch.tanh, 
        ...
        # Deleted the extra argument gnn=GNN
    ...
    def forward(self, **kwargs):
        ...
        # The two _ are for perm (describes order in which nodes were selected for pooling) and score (describes the attention score of each node)
        x, edge_index, edge_attr, batch, attn, _ = self.pool1(x, edge_index, edge_attr, batch)
        attn = attn.float()
        x1 = torch.cat([gmp(x, batch), gap(x, batch)], dim=1)
        x = F.relu(self.conv2(x, edge_index))

        x, edge_index, edge_attr, batch, attn, _ = self.pool2(x, edge_index, edge_attr, batch)
        attn = attn.float()
...
class PathNet(nn.Module):
    ...
    def forward(self, x_path, x_patname):#**kwargs):
        # x_path, x_patname = input #kwargs['x_path']
        x = self.features(x_path)
        if type(x) is tuple:
            x=x[0]
        else:
            x = self.avgpool(x)
...
# instantiates the PathNet wtih layers created by make_layers and instantiates and returns an instance of a VGG-like neural network model
def get_vgg(arch='vgg19_bn', cfg='E', act=None, batch_norm=True, label_dim=1, pretrained=True, progress=True, **kwargs):
    ...
    # Load pre-trained weights from a URL associated with the specific architecture arch
    if pretrained:
