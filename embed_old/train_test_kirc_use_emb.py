...
from data_loaders import UseEmbeddingsKircClinDatasetLoader
from embed_old.networks_kirc_use_emb import define_net, define_reg, define_optimizer, define_scheduler
from utils import (
    unfreeze_unimodal,
    CoxLoss,
    CIndex_lifeline,
...
def train(opt, data, device, k):
    ...
    custom_data_loader = (
        UseEmbeddingsKircClinDatasetLoader(opt, data, split="train", mode=opt.mode) 
    )
...
def test(opt, model, data, split, device):
    ...
    model.eval()
    custom_data_loader = (
        UseEmbeddingsKircClinDatasetLoader(opt, data, split=split, mode=opt.mode) 
    )
